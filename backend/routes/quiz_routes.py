...
from backend.services.quiz_from_text import generate_quiz_from_text
...
def generate_quiz():
    ...
    if document_id:
        ...
        if not extracted:
            # Continue with generic questions rather than failing hard
    ...
    if document_text:
        sample_questions = generate_quiz_from_text(document_text)
    else:
        # Generic fallback
        sample_questions = [
            {
